...
public interface ICache<T> {
    T Fetch(string key, Func<T> valueFactory, TimeSpan timeToLive);
    Task<T> FetchAsync(string key, Func<Task<T>> valueFactory, TimeSpan timeToLive);
}

/// <summary>
/// Cache implementation backed by OrderedDictionary. Internally, OrderedDictionary is
/// a hash combined with a list. This makes LRU eviction and promotion performance poor since
/// it means elements in the list, an O(n) operation.
/// </summary>
...
/// </remarks>
public readonly struct Cache<T>(int maxItems, Func<DateTime> nowProvider) : ICache<T> {
    ...
    private readonly OrderedDictionary<string, CacheEntry> _cache = new();

    public T Fetch(string key, Func<T> valueFactory, TimeSpan timeToLive) {
        if (EntryValue(key, out CacheEntry entry)) return entry.Value;
    ...
    private bool EntryValue(string key, out CacheEntry entry) {
        long now = nowProvider().Ticks;
        lock (_cache) {
            if (!_cache.TryGetValue(key, out entry)) {
                entry = _cache[key] = new CacheEntry(); // create/add empty entry for key
                while (_cache.Count > maxItems) _cache.RemoveAt(0); // shrink cache if necessary
            } else if (Volatile.Read(ref entry.Expires) > now) {
                if (_cache.GetAt(_cache.Count - 1).Value.Semaphore != entry.Semaphore) {
                    _cache.Remove(key); // remove and
                    _cache.Add(key, entry); // re-add key to end
                }
                return true;
            }
        }
...
/// <see href="https://github.com/tpn/pdfs/blob/master/Memory%20Barriers%20-%20a%20Hardware%20View%20for%20Software%20Hackers%20(July%2023%2C%202010).pdf"/>
/// </remarks>
public readonly struct Cache2<T>(int maxItems, Func<DateTime> nowProvider) : ICache<T> {
...
public readonly struct Cache3<TKey, TValue>(int maxItems, Func<DateTime> nowProvider, IEqualityComparer<TKey> keyComparer) : ICache3<TKey, TValue> {
    ...
    private bool EntryValue(TKey key, out CacheEntry entry) {
        long now = nowProvider().Ticks;
        lock (_cache) {
            if (!_cache.TryGetValue(key, out var node)) {
                entry = new CacheEntry(key); // create/add empty entry for key
